...
import { PROJECT_APPLICATION_REASON_MAX } from '../schemas'
import type { ProjectApplicationStatus } from '../types'

interface DecideApplicationModalProps {
  applicantName: string
  target: Extract<ProjectApplicationStatus, 'ACCEPTED' | 'REJECTED'>
  isPending: boolean
...
/**
 * Owner/ADMIN deciding on one applicant, with an optional reason surfaced in the
 * `PROJECT_APPLICATION_ACCEPTED`/`REJECTED` notification the applicant receives. Copy of Jobs'
 * `DecideApplicationModal`; destructive styling only for Reject.
 */
    ...
    >
      ...
      >
        <h3 id="decide-application-title" className="mb-1.5 text-base font-semibold text-gray-900 dark:text-white">
          {target === 'ACCEPTED' ? 'Accept' : 'Reject'} {applicantName}?
        </h3>
        <p className="mb-4 text-sm text-gray-500 dark:text-gray-400">
          {target === 'ACCEPTED'
            ? "They'll join the team and fill this role's slot."
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            maxLength={PROJECT_APPLICATION_REASON_MAX}
            placeholder="Great fit for the role, looking for someone with more experience, etc."
            value={reason}
