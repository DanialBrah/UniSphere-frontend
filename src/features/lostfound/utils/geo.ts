...
/**
 * The privacy guard rounds a FOUND item's coordinates to 2 decimal places
 * (`lost-found.coarse-coordinate-scale`). One degree of latitude is ~111 km, so 0.01° is ~1.1 km,
 * and the true point lies within half that of the rounded one. The circle we draw uses the full
 * ~1.1 km so it reads as "somewhere around here" rather than implying false precision.
 */
export const APPROXIMATE_RADIUS_METERS = 1100

/** Converts a Leaflet viewport into the four query params `/items/map` expects. */
...
export function hasCoordinates(value: {
  ...
  return value.incidentLatitude != null && value.incidentLongitude != null
}

/** Metres under 1 km, one decimal above — the same shape the backend uses in match reasons. */
