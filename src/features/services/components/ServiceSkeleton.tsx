const SURFACE = 'rounded-2xl border border-gray-200 bg-white dark:border-[#2D1F4D] dark:bg-[#1A1226]'
const SHIMMER = 'animate-pulse rounded bg-gray-200 dark:bg-white/10'

/** Matches ServiceCard's layout so the swap to real content doesn't shift the grid. */
...
export function ServiceGridSkeleton({ count = 6 }: Readonly<{ count?: number }>) {
  return (
...
export function ServiceRosterRowSkeleton() {
  return (
...
export function ServiceStatsSkeleton() {
  return (
...
export function ServiceDetailSkeleton() {
  return (
    <div className="space-y-4">
      <div className={`aspect-[21/9] w-full ${SHIMMER} rounded-2xl`} />
