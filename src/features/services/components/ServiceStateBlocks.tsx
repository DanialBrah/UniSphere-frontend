...
/** The icon + heading + hint + optional action block every list state uses. */
export function ServiceStateBlock({
  icon: Icon,
  title,
  hint,
  actionLabel,
  ...
  const iconWrapper = tone === 'error' ? 'bg-red-50 dark:bg-red-900/20' : 'bg-primary-50 dark:bg-primary/10'
  const iconColor = tone === 'error' ? 'text-red-500' : 'text-primary'

  return (
    <div className="py-16 text-center">
      <div className={`mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl ${iconWrapper}`}>
        <Icon size={28} className={iconColor} />
      </div>
      <h3 className="mb-1 text-base font-semibold text-gray-900 dark:text-white">{title}</h3>
      {hint && <p className="mx-auto mb-5 max-w-md text-sm text-gray-500 dark:text-gray-400">{hint}</p>}
...
export function ServiceErrorState({
  ...
  return (
    <ServiceStateBlock
      icon={icon}
      title={title}
      hint={serviceErrorMessage(error)}
      actionLabel="Retry"
      onAction={onRetry}
      tone="error"
    />
