import { useEffect } from 'react'
import { useNavigate } from 'react-router-dom'
import { useForm, type Resolver } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { toast } from 'sonner'
import { ArrowLeft } from 'lucide-react'
import { useRegister } from '../../hooks/useRegister'
import { studentSchema, type StudentFormData } from '../../schemas'
import { inputClass } from './formUtils'
import { FieldError, Label, PasswordInput, OptionalSection } from './FormPrimitives'
import { getErrorMessage } from '../../../../lib/utils'
import { useCooldown } from '../../../../hooks/useCooldown'

export function StudentForm({ onBack }: { onBack: () => void }) {
  const navigate = useNavigate()
  const { mutate, isPending, isSuccess, error } = useRegister('STUDENT')
  const cooldown = useCooldown(error)
  const { register, handleSubmit, formState: { errors } } = useForm<StudentFormData>({
    resolver: zodResolver(studentSchema) as unknown as Resolver<StudentFormData>,
  })

  useEffect(() => { if (isSuccess) navigate('/dashboard', { replace: true }) }, [isSuccess, navigate])
  useEffect(() => { if (error) toast.error(getErrorMessage(error)) }, [error])

  return (
    <form noValidate className="space-y-4" onSubmit={handleSubmit((d) => mutate(d))}>
      <div>
        <Label>Full name *</Label>
        <input {...register('fullName')} placeholder="Jane Smith" className={inputClass(!!errors.fullName)} />
        <FieldError message={errors.fullName?.message} />
      </div>
      <div>
        <Label>Matric number *</Label>
        <input {...register('matricNumber')} placeholder="U2021/12345" className={inputClass(!!errors.matricNumber)} />
        <FieldError message={errors.matricNumber?.message} />
      </div>
      <div>
        <Label>Email address *</Label>
        <input {...register('email')} type="email" placeholder="jane@university.edu" className={inputClass(!!errors.email)} />
        <FieldError message={errors.email?.message} />
      </div>
      <div>
        <Label>Password *</Label>
        <PasswordInput registered={register('password')} error={errors.password?.message} />
        <FieldError message={errors.password?.message} />
      </div>
      <OptionalSection>
        <div>
          <Label>University email</Label>
          <input {...register('universityEmail')} type="email" placeholder="jane@uni.edu.my" className={inputClass(!!errors.universityEmail)} />
          <FieldError message={errors.universityEmail?.message} />
        </div>
        <div>
          <Label>Phone</Label>
          <input {...register('phone')} type="tel" placeholder="+601234567890" className={inputClass(!!errors.phone)} />
          <FieldError message={errors.phone?.message} />
        </div>
        <div>
          <Label>Faculty</Label>
          <input {...register('faculty')} placeholder="Faculty of Computer Science" className={inputClass(false)} />
        </div>
        <div>
          <Label>Program</Label>
          <input {...register('program')} placeholder="Bachelor of Software Engineering" className={inputClass(false)} />
        </div>
        <div>
          <Label>Year of study</Label>
          <input {...register('yearOfStudy', { setValueAs: (v: string) => v === '' ? undefined : Number(v) })} type="number" min={1} max={10} placeholder="2" aria-invalid={!!errors.yearOfStudy} className={inputClass(!!errors.yearOfStudy)} />
          <FieldError message={errors.yearOfStudy?.message} />
        </div>
      </OptionalSection>
      <div className="flex gap-3 pt-2">
        <button type="button" onClick={onBack} className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-[#2D1F4D] text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
          <ArrowLeft size={14} /> Back
        </button>
        <button type="submit" disabled={isPending || cooldown > 0} className="flex-1 py-2.5 bg-primary-600 hover:bg-primary-700 disabled:opacity-50 text-white font-semibold rounded-xl text-sm transition-colors flex items-center justify-center gap-2">
          {isPending && <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />}
          {cooldown > 0 ? `Try again in ${cooldown}s` : isPending ? 'Creating account…' : 'Create account'}
        </button>
      </div>
    </form>
  )
}
